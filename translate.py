...
def sample(model, dictionary, dictionary_target, \
    ...
    bleu_score = gen_trans(test_src=source_file, test_ref=ref_file, out_file=saveto, \
                           ...
                           trng=trng, k=10, stochastic=False, bleu_script=bleu_script, res_to_sgm=res_to_sgm, apply_bpe=True)
...
if __name__ == "__main__":
    ...
    for model in test_model:
        ...
        if os.path.isfile(model) == False:
            #print('file [{}] do not exist'.format(model))
            continue
        for i in range(len(test_file)):
            if os.path.isfile(test_file[i][0]) == False:
                #print('file [{}] do not exist'.format(test_file[i][0]))
                continue
